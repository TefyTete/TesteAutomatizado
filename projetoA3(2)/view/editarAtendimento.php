...
<body class="bg-secondary">
    <div class="container bg-light p-0" style="width:800px; margin: auto">
        ...
        <form method="post" action="../controller/atendimentoController.php?action=editarAtendimento">
            ...
            <div class="m-3">
                <label for="idpublico" class="form-label">Público:</label>
                <select required id="idpublico" name="idpublico" class="form-control">
                    ...
                    $consultapublico = $publicoController->consulta();
                    foreach ($consultapublico as $linha) {
                        $idpublico = $linha['idpublico'];
                        $nomepublico = $linha['nomepublico'];
                        $selected = ($atendimento->getIdpublico() == $idpublico) ? 'selected' : '';
                        echo "<option value='$idpublico' $selected>$nomepublico</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <div class="m-3">
                <label for="respostaAtendimento" class="form-label">Identificação do Atendido:</label>
